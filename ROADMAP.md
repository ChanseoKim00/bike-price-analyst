...
- 마이페이지 (`/mypage`) — 탭 구조 (일반·계정·결제·사용량·검색 히스토리)
  - 일반 탭: 프로필 조회, 이메일 알림 수신 설정
  - 계정 탭: 닉네임 변경 / 이메일 변경 (로컬 계정) / 비밀번호 변경 (로컬 계정)
...
- 차분하고 친절한 전화 상담원 말투, 마크다운 미사용, 역할 탈출 시도 가드레일 적용

### ✅ Phase 10 — 관리자 대시보드 고도화 (완료)
- `/admin/users` — 일별 신규 가입자 추이 선 차트 (최근 30일) + 전체 회원 목록
- `/admin/analyses` — 일별 분석 건수 추이 선 차트 (최근 30일) + 최근 분석 목록 (최대 200건)
- `/admin/feedbacks` — 일별 평균 만족도 선 차트 + 점수(1~10)별 분포 막대그래프 (최근 30일) + 피드백 목록
- 메인 대시보드 KPI 카드 — 오늘 신규 가입자, 플랜 분포, 오늘 분석 건수, 게스트/회원 분석 비율, 평균 만족도, full/quick 피드백 수
- 이탈 팝업 쿨타임 14일(336시간)로 조정 (기존 72시간 → 14일)
